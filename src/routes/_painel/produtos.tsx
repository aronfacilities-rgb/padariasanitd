...
function ProdutosPage() {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault(); const f = new FormData(e.currentTarget); const nome = String(f.get("nome") ?? "").trim(); if (!nome) return toast.error("Informe o nome do produto");
    ...
    const precoKg = tipo === "PESO" ? Number(f.get("preco_kg") ?? 0) : null; const plu = tipo === "PESO" ? (String(f.get("plu") ?? "").replace(/\D/g, "").padStart(5, "0") || null) : null;
    if (tipo === "PESO" && (!plu || plu.length !== 5)) return toast.error("Informe o PLU com 5 dígitos para o produto de peso.");
    if (tipo === "PESO" && !(precoKg > 0)) return toast.error("Informe o preço por kg do produto de peso.");
    salvar.mutate({ id: editando?.id ?? null, dados: { nome, codigo_interno: codigoInterno, codigo_barras: barras || gerarEan13(codigoInterno), category_id: String(f.get("category_id") ?? "") !== "sem" ? String(f.get("category_id") ?? "") || null : null, unidade: tipo === "PESO" ? "KG" : String(f.get("unidade") ?? "UN"), preco_venda: tipo === "PESO" ? precoKg : Number(f.get("preco_venda") ?? 0), preco_kg: precoKg, tipo_venda: tipo, plu, custo: Number(f.get("custo") ?? 0), estoque_minimo: Number(f.get("estoque_minimo") ?? 0), ncm: String(f.get("ncm") ?? "") || null, cfop: String(f.get("cfop") ?? "") || null, cst: String(f.get("cst") ?? "") || null, aliquota: f.get("aliquota") ? Number(f.get("aliquota")) : null } });
  }
  return <><PageHeader title="Produtos" description="Cadastro, preços, estoque e códigos dos produtos." actions={isManager ? <Button className="h-11" onClick={() => { setEditando(null); setAberto(true); }}><Plus className="size-4" /> Novo produto</Button> : null} />
    ...
    <Dialog open={aberto} onOpenChange={(v) => { setAberto(v); if (!v) setEditando(null); }}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle className="font-display">{editando ? "Editar produto" : "Novo produto"}</DialogTitle></DialogHeader><form key={editando?.id ?? "novo"} onSubmit={onSubmit} className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="space-y-2"><Label htmlFor="plu">PLU {editando?.tipo_venda === "PESO" ? "(5 dígitos)" : ""}</Label><Input id="plu" name="plu" defaultValue={editando?.plu ?? ""} placeholder="Ex.: 00600" inputMode="numeric" /></div>
