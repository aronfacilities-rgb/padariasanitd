...
function PainelLayout() {
  ...
  useEffect(() => { if (!loading && !session) void navigate({ to: "/auth" }); }, [loading, session, navigate]);
  ...
  if (roles.length === 0) return <div className="flex min-h-screen items-center justify-center px-6"><div className="panel max-w-md p-8 text-center"><ShieldCheck className="mx-auto size-8 text-muted-foreground" /><h1 className="mt-4 font-display text-xl">Acesso aguardando liberação</h1><p className="mt-2 text-sm text-muted-foreground">Sua conta foi criada, mas ainda não possui permissões. Peça à administração para liberar seu perfil de acesso.</p><Button variant="outline" className="mt-6" onClick={() => void signOut()}>Sair</Button></div></div>;
