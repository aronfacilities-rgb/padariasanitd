...
function PainelLayout() {
  const { session, loading, profile, roles, signOut } = useAuth(); const navigate = useNavigate(); const pathname = useRouterState({ select: (s) => s.location.pathname }); const [menuOpen, setMenuOpen] = useState(false);
  useEffect(() => { if (!loading && !session) void navigate({ to: "/auth" }); }, [loading, session, navigate]);
  const visible = NAV.filter((item) => !item.roles || item.roles.some((r) => roles.includes(r))); const mobileItems = visible.filter((i) => MOBILE_KEYS.includes(i.to)).slice(0, 4);
